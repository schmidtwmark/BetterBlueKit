//
//  KiaAustraliaAPIClient.swift
//  BetterBlueKit
//
//  Kia Australia API Client (Stub)
//

import Foundation

// MARK: - Kia Australia API Client

@MainActor
public final class KiaAustraliaAPIClient: APIClientBase, APIClientProtocol {

    public override var apiName: String { "KiaAustralia" }

    // MARK: - APIClientProtocol Implementation (Stubs)

    public func login() async throws -> AuthToken {
        throw APIError.regionNotSupported(
            "Kia Australia is not yet implemented. " +
    ...
    public func fetchVehicles(authToken: AuthToken) async throws -> [Vehicle] {
        throw APIError.regionNotSupported("Kia Australia is not yet implemented.", apiName: apiName)
    }
